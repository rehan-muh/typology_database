...
for fn in sys.argv[1:]:
 p=Path(fn); o=json.loads(p.read_text(encoding='utf-8')); raw=gzip.decompress(base64.b64decode(o['payload_base64']))
 ...
 rows.extend(rr)
ids=[r['id'] for r in rows]; dks=[r['dedup_key'] for r in rows]; s=set(ids)
if len(ids)!=len(s): errors.append(['duplicate_ids'])
if len(dks)!=len(set(dks)): errors.append(['duplicate_dedup_keys'])
broken=[(r['id'],x) for r in rows for x in r.get('dependency_links',[]) if x not in s]
